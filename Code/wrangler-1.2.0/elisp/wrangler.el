...
(defvar refactor-menu-items
  `(("Rename Variable Name" erl-refactor-rename-var)
    ...
    nil
    ("Refactorings for Parallelisation"
    (
     ("Task Chunking"  erl-refactor-task-chunking)
     ("Farm Introduction" erl-refactor-farm-intro)
     ("Map Introduction"  erl-refactor-map-intro)
     ("Pipe Introduncton" erl-refactor-pipe-intro)
    ))
    nil
    ("Refactorings for QuickCheck"  
     ...
     ;; ("eqc_fsm State Data to Record" erl-refactor-eqc-fsm-to-record)
      ...
      ("Rename a WS opertation argument" erl-refactor-rename-op-arg)
      ("Add a WS opertation argument" erl-refactor-add-op-arg)
      ("Remove a WS opertation argument" erl-refactor-remove-op-arg)  
      ("Add a WS opertation" erl-refactor-add-op)
      ("Add a WS opertation" erl-refactor-add-op)       
      ("Remove a WS opertation" erl-refactor-remove-op) 
      ("Re-order WS operation parameters" erl-refactor-swap-op-args)           
      ))
    nil
    ...
    nil
    ("Behaviour Refactorings (Beta)"
     (
      ("Function to behaviour instance" erl-refactor-func-to-behav-instance)
      ("Expression to behaviour instance" erl-refactor-expr-to-behav-instance)
      ("Unfold behaviour instance" erl-refactor-unfold-behav-instance)
      nil
      ("Infer behaviour from two modules" erl-refactor-infer-behaviour)
      nil
      ("Copy Module" erl-refactor-copy-mod)
      ("Create behaviour instance file" refac_create_behav_instance)
      ("Add function to behaviour_info" refac_add_callback)
      ("Add function name to behaviour_info" refac_add_callbacks)
      ("Remove behaviour_info function" refac_remove_behav_dec)
      ("Instantiate dynamic module from calls" refac_instantiate_calls)
      ))
    nil
...
(defvar inspector-menu-items
  '(("Instances of a Variable" erl-wrangler-code-inspector-var-instances) 
    ...
    ("Backward intra-function slice" erl-wrangler-code-inspector-backward-slice) 
    nil
...
(defun erl-refactor-version()
  (interactive)
  (message "Wrangler version 1.2.0")) 

(setq wrangler-version  "(wrangler-1.2.0) ")
...
(defun wrangler-menu-init()
  ...
  (define-key erlang-mode-map "\C-c\C-wx" 'erl-wrangler-code-inspector-backward-slice)  
  ...
  (define-key erlang-mode-map "\C-c\C-wg"    'erl-refactor-generalisation)
...
(defun erl-refactor-copy-mod (name)
  ...
  (if (buffers-saved)
      (erl-spawn
	(erl-send-rpc wrangler-erl-node 'emacs_wrangler 'apply_refac 
                      (list 'copy_mod (list current-file-name name wrangler-search-paths 'emacs tab-width)
                            wrangler-search-paths))
        (erl-receive (buffer name current-file-name)
	    ((['rex ['badrpc rsn]]
	      (message "Refactoring failed: %S" rsn))
		...
		(if (y-or-n-p msg)
		    (erl-spawn
		      (erl-send-rpc wrangler-erl-node 'wrangler_refacs 'try_refac
                                    (list 'refac_copy_mod 'copy_mod_1
                                          (list current-file-name name wrangler-search-paths tab-width 'false 'emacs)))
		      (erl-receive (current-file-name)
			  ((['rex ['badrpc rsn]]
			    (message "Refactoring failed: %S" rsn))
			   (['rex ['error rsn]]
			    (message "Refactoring failed: %s" rsn))
			   (['rex ['ok modified copied  warning]]
			    (progn 
			      (setq has-warning warning)
			      (preview-commit-cancel current-file-name modified copied)
			    )))
		      (message "Refactoring aborted.")
		      )))))
	     (['rex ['question msg]]
...
(defun erl-refactor-infer-behaviour(leftmod rightmod destmod)
  "Infer a new behaviour from two similar modules."
 (interactive (list (read-string "First module to compare: ")
		    (read-string "Second module to compare: ")
		    (read-string "Dest module (must not exist):")
		    ))
  (let ((current-file-name (buffer-file-name)))
    (erl-spawn
	 (erl-send-rpc wrangler-erl-node 'wrangler_refacs 'try_refac (list 'infer_behaviour 'infer_behaviour
				    (list leftmod rightmod destmod wrangler-search-paths 'emacs tab-width)))
	  (erl-receive (current-file-name)
	      ((['rex ['badrpc rsn]]
		(message "Refactoring failed: %S" rsn))
	       (['rex ['error rsn]]
...
(defun refac_add_callback()
  (interactive)
  (apply-adhoc-refac 'refac_add_callback))

(defun refac_add_callbacks()
  (interactive)
  (apply-adhoc-refac 'refac_add_callbacks))

(defun refac_create_behav_instance()
  (interactive)
  (apply-adhoc-refac 'refac_create_behav_instance))

(defun refac_remove_behav_dec()
  (interactive)
  (apply-adhoc-refac 'refac_remove_behav_dec))

(defun erl-refactor-func-to-behav-instance()
  (interactive)
  (apply-composite-refac 'refac_func_to_behav_instance))

(defun erl-refactor-expr-to-behav-instance()
  (interactive)
  (apply-composite-refac 'refac_expr_to_behav_instance))

(defun erl-refactor-unfold-behav-instance()
  (interactive)
  (apply-composite-refac 'refac_unfold_behav_instance))

(defun refac_instantiate_callback()
  (interactive)
  (apply-adhoc-refac 'refac_instantiate_calls))
...
(defun erl-refactor-rename-op-arg()
  (interactive)
  (apply-composite-refac 'refac_rename_op_arg))

(defun erl-refactor-add-op-arg()
  (interactive)
  (apply-adhoc-refac 'refac_add_op_arg))

(defun erl-refactor-remove-op-arg()
  (interactive)
  (apply-adhoc-refac 'refac_rm_op_arg))

(defun erl-refactor-add-op()
  (interactive)
  (apply-adhoc-refac 'refac_add_op))

(defun erl-refactor-remove-op()
  (interactive)
  (apply-adhoc-refac 'refac_rm_op))

(defun erl-refactor-swap-op-args()
  (interactive)
  (apply-adhoc-refac 'refac_swap_op_args))
...
(defun erl-wrangler-code-inspector-backward-slice()
  "Intra-function backward slice"
  (interactive)
  (let* ((buffer (current-buffer))
    ...
    (save-buffer)
    (erl-spawn
      (erl-send-rpc wrangler-erl-node 'wrangler_slice_new 'backward_intra_fun_slice 
			  (list current-file-name start-line-no start-col-no 
                          end-line-no end-col-no wrangler-search-paths tab-width))
      (erl-receive (buffer)
	  ((['rex ['badrpc rsn]]
	    (message "Error: %S" rsn))
	   (['rex ['error rsn]]
	    (message "Error: %s" rsn))
	   (['rex ['ok regions maybe-ranges defpos]]
	    (with-current-buffer buffer 
            (highlight-instances regions defpos buffer)
            (highlight-maybe-instances maybe-ranges defpos buffer)
            (message "\nUse 'C-c C-w e' to remove highlights.\n")
	                         	          )
...
(defun highlight-maybe-instances(regions defpos buffer)
  "highlight regions in the buffer"
  (dolist (r regions)
      (highlight-maybe-instance r buffer)))
...
(defun highlight-maybe-instance(region buffer)
   "highlight one region in the buffer"
   (let ((line1 (elt (elt region 0) 0))
	  (col1 (elt (elt region 0) 1))
	  (line2 (elt (elt region 1) 0))
	  (col2 (elt (elt region 1) 1)))
     (highlight-region-with-face line1 col1 line2 col2 buffer 'maybe-instance-face)))
...
(defun erl-wrangler-code-inspector-caller-called-mods()
  "search for the dependencies of a module"
  (interactive)
  (let ((current-file-name (buffer-file-name))
	(line-no           (current-line-no))
        (column-no         (current-column-no))
...
("Inline Variable"  refac_inline_variable)
("Symbolic Evaluation"  (gen_refac_other_items))
))

(defvar gen_refac_other_items
`(("Arithmetic Operators" refac_arithmetics)
("Boolean Operators" refac_boolean_operators)
("Case" refac_case)
("If" refac_if)
("Lists Concatenation" refac_lists_concat)
("Remove Unreferenced Assignments" refac_unreferenced_assign)
("Unfold Function Application" refac_funApp)
("All" refac_all)
))
...
(defun refac_arithmetics()
  (interactive)
  (apply-adhoc-refac 'refac_arithmetics))

(defun refac_boolean_operators()
  (interactive)
  (apply-adhoc-refac 'refac_boolean_operators))

(defun refac_case()
  (interactive)
  (apply-adhoc-refac 'refac_case))

(defun refac_if()
  (interactive)
  (apply-adhoc-refac 'refac_if))

(defun refac_lists_concat()
  (interactive)
  (apply-adhoc-refac 'refac_lists_concat))

(defun refac_funApp()
  (interactive)
  (apply-adhoc-refac 'refac_funApp))

(defun refac_unreferenced_assign()
  (interactive)
  (apply-adhoc-refac 'refac_unreferenced_assign))

(defun refac_all()
  (interactive)
  (apply-adhoc-refac 'refac_all))

(defun refac_inline_variable()
  (interactive)
  (apply-adhoc-refac 'refac_inline_variable))

(defun erl-refactor-task-chunking()
  (interactive)
  (apply-adhoc-refac 'refac_chunking))


(defun erl-refactor-farm-intro()
  (interactive)
  (apply-adhoc-refac 'refac_farm_intro))


(defun erl-refactor-map-intro()
  (interactive)
  (apply-adhoc-refac 'refac_map_intro))


(defun erl-refactor-pipe-intro()
  (interactive)
  (apply-adhoc-refac 'refac_pipe_intro))


(defvar gen_composite_refac_menu_items
`(("Func To Behav Instance" refac_func_to_behav_instance)
("Batch Inline Vars" refac_batch_inline_vars)
("Batch Clone Elimination" refac_batch_clone_elimination)
("Batch Prefix Module" refac_batch_prefix_module)
("Rename Op Arg" refac_rename_op_arg)
("Expr To Behav Instance" refac_expr_to_behav_instance)
("Unfold Behav Instance" refac_unfold_behav_instance)
))

(defvar gen_refac_menu_items
`(("Create Behav Instance" refac_create_behav_instance)
("Remove An Import Attribute" refac_remove_an_import_attribute)
("Add To Export" refac_add_to_export)
("Rm Op Arg" refac_rm_op_arg)
("Rm Op" refac_rm_op)
("Farm Intro" refac_farm_intro)
("Swap Op Args" refac_swap_op_args)
("Chunking" refac_chunking)
("Pipe Intro" refac_pipe_intro)
("Instantiate Calls" refac_instantiate_calls)
("Add Op" refac_add_op)
("Remove An Argument" refac_remove_an_argument)
("Remove Behav Dec" refac_remove_behav_dec)
("Force Remove Arg" refac_force_remove_arg)
("Swap Function Arguments" refac_swap_function_arguments)
("Add Op Arg" refac_add_op_arg)
("Specialise A Function" refac_specialise_a_function)
("Add Argument" refac_add_argument)
("Add Callback" refac_add_callback)
("Apply To Remote Call" refac_apply_to_remote_call)
("Add Callbacks" refac_add_callbacks)
("Add An Import Attribute" refac_add_an_import_attribute)
("Map Intro" refac_map_intro)
))


(defun refac_func_to_behav_instance()
  (interactive)
  (apply-composite-refac 'refac_func_to_behav_instance))
...
(defun refac_rename_op_arg()
  (interactive)
  (apply-composite-refac 'refac_rename_op_arg))


(defun refac_expr_to_behav_instance()
  (interactive)
  (apply-composite-refac 'refac_expr_to_behav_instance))


(defun refac_unfold_behav_instance()
  (interactive)
  (apply-composite-refac 'refac_unfold_behav_instance))
...
(defun refac_farm_intro()
  (interactive)
  (apply-adhoc-refac 'refac_farm_intro))


(defun refac_swap_op_args()
  (interactive)
  (apply-adhoc-refac 'refac_swap_op_args))


(defun refac_chunking()
  (interactive)
  (apply-adhoc-refac 'refac_chunking))


(defun refac_pipe_intro()
  (interactive)
  (apply-adhoc-refac 'refac_pipe_intro))


(defun refac_instantiate_calls()
  (interactive)
  (apply-adhoc-refac 'refac_instantiate_calls))
...
(defun refac_force_remove_arg()
  (interactive)
  (apply-adhoc-refac 'refac_force_remove_arg))
...
(defun refac_add_argument()
  (interactive)
  (apply-adhoc-refac 'refac_add_argument))
